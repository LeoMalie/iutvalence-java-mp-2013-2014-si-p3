...
public class Area
{
    ...
    /**
     * This method provides to change the value of a content (EMPTY/USERCAR/BOTCAR)
     * @param pos the content position this function use
     * @param s the new content that this function have to put in the square
     */
        ...
        for(int y = 0; y < SIZE_HEIGHT; y++)
        {
            for(int x = 0; x < SIZE_WIDTH; x++)
            {
                switch(this.getContent(x,y)) 
                {
                    ...
                    default : testArea = testArea + "?"; break;
                }  
            }
            testArea = testArea + "\n";
        }
